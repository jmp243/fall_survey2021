...
names(upp_c)
upp_c<-upp_c[,-c(1,3,10:12,15,17,18)]

upp_c %>% group_by(Profile.Name) %>% tally 
...
df_foruse<-merge(upp_c2, affiliation1s, by.x = "Id.y", by.y = "hed__Contact__c", all.x = TRUE)

names(df_foruse)
df_foruse<-df_foruse[,-c(1,2,11,13,14)]
...
for_reshape %>% group_by(Profile.Name) %>% tally 
...
write.csv(for_use,"20220304_SFTrellisUsersByProduct.csv")

#### load in the list locally ####
